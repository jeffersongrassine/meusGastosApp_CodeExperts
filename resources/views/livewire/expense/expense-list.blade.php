        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="px-4 py-2 border">
                    <span class="{{ $exp->type == 1 ? 'text-blue-800' : 'text-red-800' }}">
                        R$ {{number_format($exp->amount, 2, ',', '.')}}
                    </span>
                </td>
                <td class="px-4 py-2 border">{{$exp->created_at->format('d/m/Y H:i:s')}}</td>
                <td class="px-4 py-4 border">
                    <a href="{{route('expenses.edit', $exp->id)}}" class="px-4 py-2 border rounded bg-blue-500 hover:bg-blue-700 text-white">Editar</a>
                    <a href="#" wire:click.prevent="remove({{$exp->id}})"
                    class="px-4 py-2 border rounded bg-red-500 hover:bg-red-700 text-white">Remover</a>
                </td>
            </tr>
            @endforeach
        </tbody>
